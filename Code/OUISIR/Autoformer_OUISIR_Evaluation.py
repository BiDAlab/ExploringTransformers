...
import argparse
from Models_Transformers import Transformer_THAT, PositionalEncoding, AutoformerAutoformer, InformerAutoformer, Gaussian_Position, HARTransformerAutoformer, HARInformerAutoformer, HARAutoformerAutoformer, HAR_BRTransformer, TransformerAutoformer
...
class Autoformer(torch.nn.Module):
    def __init__(self, args):
        ...
        self.transformer = AutoformerAutoformer(128, args.hlayers, args.hheads, 6)
        self.args = args
        self.kernel_num = 512
...
TransformerModel = Autoformer(configs) 
TransformerModel = TransformerModel.to(device)
############################################################################
# TESTING
TransformerModel.eval()
TransformerModel.load_state_dict(torch.load('Autoformer_OUISIR'))
...
print('Accuracy Autoformer: {}/{} ({:.3f}%)'.format(
