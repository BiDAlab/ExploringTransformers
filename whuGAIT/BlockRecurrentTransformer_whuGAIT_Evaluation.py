...
import argparse
from Models_Transformers import Transformer_THAT, PositionalEncoding, BlockRecTransformer, AutoformerAutoformer, HAR_BRTransformer, InformerAutoformer, Gaussian_Position, HARTransformerAutoformer, HARInformerAutoformer, HARAutoformerAutoformer, HARTransformer, TransformerAutoformer
...
class BlockRecurrentTransformer(torch.nn.Module):
    def __init__(self, args):
        super(BlockRecurrentTransformer, self).__init__()
        self.softmax = torch.nn.LogSoftmax(dim=1)
        self.transformer_pre = TransformerAutoformer(80, 9, args.hheads, 6)
        self.transformer = BlockRecTransformer(80, 1, args.hheads, 6)
        self.transformer_pos = TransformerAutoformer(80, 2, args.hheads, 6)
        self.args = args
        self.kernel_num = 512
        self.kernel_num_v = 512
...
TransformerModel = BlockRecurrentTransformer(configs) 
TransformerModel = TransformerModel.to(device)
############################################################################
# TESTING
TransformerModel.load_state_dict(torch.load('BlockRecurrent_whuGAIT'))
...
print('Accuracy Block-Recurrent Transformer: {}/{} ({:.3f}%)'.format(
